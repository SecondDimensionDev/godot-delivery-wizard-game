...
### Manages the player's weapon inventory and input routing.
###
### This component listens for player input to swap weapons, fire, and reload.
### It handles instantiating the correct weapon scene into the WeaponRig and
### passing the AmmoComponent via dependency injection.
#
## SIGNALS
#signal weapon_switched(weapon_resource: FPSWeaponData, weapon_node: BaseWeapon) ## Emitted when the active weapon changes.
#
...
#func _unhandled_input(event: InputEvent) -> void:
	#if not is_instance_valid(_active_weapon_node) or _is_changing_weapon:
		#return
		#
	## Handle Reloading
...
#func _process(_delta: float) -> void:
	## We use _process for firing so the player can hold down the trigger for automatic weapons
	#if Input.is_action_pressed("fire") and is_instance_valid(_active_weapon_node):
		#_active_weapon_node.primary_fire()
#
## PUBLIC FUNCTIONS
#func equip_weapon(index: int) -> void: ## Instantiates a weapon from the inventory and makes it active.
	#if index < 0 or index >= _unlocked_weapons.size() or _is_changing_weapon:
	...
	#
	## 4. Inject the data into the weapon shell so it knows its stats
	#_active_weapon_node.initialize_weapon(new_weapon_data)
	#
	## 5. Parent it to the WeaponRig (This applies all your sway/bob/tilt automatically)
	#if is_instance_valid(weapon_rig):
		#weapon_rig.add_child(_active_weapon_node)
