...
## A generic damage source designed to interact with [DamageReceiver].
##
...
# SIGNALS
signal hit_landed(receiver: DamageReceiver) ## Emitted when this DamageDealer successfully damages a DamageReceiver.
...
@export_flags_2d_physics var combat_physics_layer: int = 4 ## The physics layer reserved for DamageDealer/DamageReceiver interactions.
@export var collide_with_terrain: bool = false
@export_flags_2d_physics var terrain_physics_layer: int = 1 ## The physics layer your solid scenery is on.
...
# PUBLIC FUNCTIONS
func confirm_hit(receiver: DamageReceiver) -> void: ## Called by the DamageReceiver to confirm damage was applied.
...
func get_modified_damage() -> int: ## Calculates final damage after applying all modifiers
...
func _setup_physics_collision() -> void:
	...
	# The DamageDealer acts as the "Key". It doesn't need to scan, just be found.
	# Note: Set monitoring to true if you need this DamageDealer to ALSO detect walls/terrain.
	...
	if collide_with_terrain:
		...
		collision_mask &= ~terrain_physics_layer
	#set_collision_layer_value(combat_physics_layer, true)
	#set_collision_mask_value(terrain_physics_layer, collide_with_terrain)
