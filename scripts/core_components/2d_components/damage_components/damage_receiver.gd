...
## A detection zone that accepts damage from [DamageDealer] Components.
##
## This component requires a child CollisionShape2D. It actively monitors for any 
## overlapping [DamageDealer] on the specified physics layer. When a valid hit is 
...
@export_flags_2d_physics var combat_physics_layer: int = 4 ## The physics layer reserved for DamageDealer/DamageReciever interactions.
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if not hit_point_component:
		warnings.append("This DamageReciever requires a HitPointComponent to function.")
	return warnings
...
func _setup_physics_collision() -> void:
	...
	# The DamageReciever acts as the "Lock" that checks the "Keys" (DamageDealers)
	monitoring = true
	monitorable = true # We stay monitorable so other systems can find us if needed
	
	# 3. Set the Mask (What we look for)
	# We listen for objects on the combat layer
	#set_collision_mask_value(combat_physics_layer, true)
