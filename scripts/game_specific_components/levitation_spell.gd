...
func _build_beam_mesh(from: Vector3, to: Vector3, _delta: float) -> void:
	...
	var view := Vector3.UP
	if player_camera:
		# Calculate view from the center of the beam to the camera
		view = (player_camera.global_position - ((from + to) * 0.5)).normalized()
		
	beam_mesh.clear_surfaces()
	beam_mesh.surface_begin(Mesh.PRIMITIVE_TRIANGLE_STRIP)
	
	# Make the physical mesh wide enough to act as a canvas for the shader
	# You can expose this as an @export variable if you want to adjust the bounds
	#var canvas_width := 0.6 
	
	for i in beam_segments + 1:
		# t goes from 0.0 to 1.0 along the length of the beam
		var t := float(i) / float(beam_segments)
		...
		# LEFT VERTEX: Set UV (x=0, y=t), then add vertex
		beam_mesh.surface_set_uv(Vector2(0.0, t))
		beam_mesh.surface_add_vertex(p - side * canvas_width)
		
		# RIGHT VERTEX: Set UV (x=1, y=t), then add vertex
		beam_mesh.surface_set_uv(Vector2(1.0, t))
		beam_mesh.surface_add_vertex(p + side * canvas_width)
